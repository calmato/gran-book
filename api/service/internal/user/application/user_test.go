	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(context context.Context, t *testing.T, mocks *test.Mocks) {
				mocks.UserRepository.EXPECT().
					Authentication(ctx).
					Return("", exception.ErrUnauthorized.New(test.ErrMock))
			},
			want: want{
				user: nil,
				err:  exception.ErrUnauthorized.New(test.ErrMock),
			},
		},
		{
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(context context.Context, t *testing.T, mocks *test.Mocks) {
				...
				mocks.UserRepository.EXPECT().
					Count(ctx, gomock.Any()).
					Return(0, exception.ErrInDatastore.New(test.ErrMock))
			},
			args: args{
				query: &database.ListQuery{},
			},
...
func TestUserApplication_GetUserProfile(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			want: want{
				user: nil,
				err:  exception.ErrNotFound.New(test.ErrMock),
			},
		},
		{
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(context context.Context, t *testing.T, mocks *test.Mocks) {
				mocks.UserDomainValidation.EXPECT().
					User(ctx, gomock.Any()).
					Return(exception.ErrInvalidDomainValidation.New(test.ErrMock))
			},
			args: args{
				user: &user.User{
					Username: "test-user",
			...
			want: want{
				err: exception.ErrInvalidDomainValidation.New(test.ErrMock),
			},
		},
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(context context.Context, t *testing.T, mocks *test.Mocks) {
				...
				mocks.UserRepository.EXPECT().
					GetRelationshipIDByUserID(ctx, "current-user", "user01").
					Return(0, exception.ErrNotFound.New(test.ErrMock))
			},
			args: args{
				userID:     "current-user",
				followerID: "user01",
