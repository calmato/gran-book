...
router.patch('/', async (req: Request<IUpdateAuthRequest>, res: Response<IAuthResponse>, next: NextFunction): Promise<void> => {
  const { username, gender, thumbnail, selfIntroduction, lastName, firstName, lastNameKana, firstNameKana, phoneNumber } = req.body as IUpdateAuthRequest

  await getAuth(req)
    .then((): Promise<IAuthOutput> => {
    ...
    .then((output: IAuthOutput): Promise<IAuthOutput> => {
