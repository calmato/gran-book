...
import { getBook, getBookByIsbn, getBookshelf, getUser, listBookReview, listBookshelf, multiGetUser } from '~/api'
import {
  IGetBookByIsbnInput,
  IGetBookInput,
  IGetBookshelfInput,
  ...
  async (req: Request, res: Response<IBookshelfV2Response>, next: NextFunction): Promise<void> => {
    ...
    await getUser(req, userInput)
      .then(async () => {
        ...
        const usersOutput = await multiGetUser(req, usersInput).catch(() => {
          return {}
        })
