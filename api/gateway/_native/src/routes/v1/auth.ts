  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    await uploadAuthThumbnail(req, input)
      .then((output: IAuthThumbnailOutput) => {
        const response: IAuthThumbnailResponse = setAuthThumbnailResponse(output)
        res.status(200).json(response)
      })
      .catch((err: Error) => {
        next(err)
...
function setAuthThumbnailResponse(output: IAuthThumbnailOutput): IAuthThumbnailResponse {
  const response: IAuthThumbnailResponse = {
    thumbnailUrl: output.thumbnailUrl,
  }
