  ...
  return new Promise((resolve: (res: IAuthThumbnailOutput) => void, reject: (reason: Error) => void) => {
    const call: ClientWritableStream<UploadAuthThumbnailRequest> = authClient.uploadAuthThumbnail(
      ...
      (err: any, res: AuthThumbnailResponse) => {
        ...
        return resolve(setAuthThumbnailOutput(res))
      }
    )
...
function setAuthOutput(res: AuthResponse): IAuthOutput {
  const auth: AuthResponse.AsObject = res.toObject()

  const output: IAuthOutput = { ...auth }
  return output
}

function setAuthThumbnailOutput(res: AuthThumbnailResponse): IAuthThumbnailOutput {
  const thumbnail: AuthThumbnailResponse.AsObject = res.toObject()

  const output: IAuthThumbnailOutput = { ...thumbnail }
  return output
}
